  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1000));
        ...
        if(typeof data.answer === 'object' && data.answer !== null && data.answer.error){
          botText = data.answer.error; //error message from the API
        }
        else{
          botText = data.answer //successful response from the API
        }
